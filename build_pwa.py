# -*- coding: utf-8 -*-
"""
Build de la PWA Geonotas Light (Track A, variante simplificada A/B/C).

Reinyecta modelo_canonico.json en el index.html EXISTENTE (index.html es la
fuente de verdad; se edita a mano, incluida su parte JS/CSS), regenera
guia_fields.js llamando a generar_guia.py, y genera manifest.json e iconos PNG
minimos si faltan. sw.js es de mantencion manual (ver escribir_sw). Sin Node.
Solo stdlib. Correr tras (re)generar el modelo canonico: es idempotente, no toca
...
- NO traduce saltos de linea. Se abre con newline="" al leer Y al escribir, y el
  ...
  cuanto el modelo cambiaba (medido: las 8579 lineas de este index.html), lo que
...
- LLAMA a generar_guia.py. guia_fields.js sale del MISMO modelo canonico: si el
  modelo cambia y solo se corre este build, el formulario A/B/C se queda con la
  guia vieja mientras el modelo embebido ya es el nuevo, sin que nada avise.
"""
...
def regenerar_guia():
    """guia_fields.js sale de generar_guia.py leyendo el MISMO modelo canonico.
    Se regenera aca para que no pueda quedar desfasado del modelo embebido."""
    gen = os.path.join(HERE, "generar_guia.py")
    if not os.path.exists(gen):
        raise SystemExit("ERROR: falta generar_guia.py; guia_fields.js quedaria desfasado\n"
                         "       del modelo. No se sigue.")
    r = subprocess.run([sys.executable, gen], cwd=HERE)
    if r.returncode != 0:
        raise SystemExit("ERROR: generar_guia.py fallo (codigo %d): guia_fields.js puede\n"
                         "       haber quedado desfasado del modelo." % r.returncode)
...
def escribir_manifest():
    manifest = {
        "name": "PWA Geonotas Light — Captura simplificada",
        "short_name": "Geonotas Light",
        "description": "Libreta digital de geología básica en terreno, offline (captura simplificada A/B/C, mapa satelital y export CSV/KMZ/GeoPackage/GDB/PDF).",
...
def escribir_sw():
    ...
    # conoce vendor/ ni guia_fields.js, asi que la app quedaria sin offline real.
    # Antes el bootstrap ademas no traia 'activate' ni 'fetch' -- cacheaba y no
    # servia nada.
    ...
    with open(p, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(
            ...
            "// Incompleto a proposito: no lista vendor/ ni guia_fields.js.\n"
            "// Completar ASSETS y subir CACHE a mano.\n"
            "const CACHE='geonotas-light-bootstrap-v0';\n"
            "const ASSETS=['./','./index.html','./manifest.json','./guia_fields.js'];\n"
